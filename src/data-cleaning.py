...
def clean_deliveries(deliveries_db_path: str):
    ...
    deliveries_df['fechaventa'] = pd.to_datetime(deliveries_df['fechaventa'], infer_datetime_format=True)
    deliveries_df['fechaenrutada'] = pd.to_datetime(deliveries_df['fechaenrutada'], infer_datetime_format=True)

    # Keep only the latest deliveries
    deliveries_df = deliveries_df[deliveries_df['fechaenrutada'] == deliveries_df['fechaenrutada'].max()]
    ...
    deliveries_df['full_address'] = deliveries_df['nombrecalle'].str.strip().str.lower() + ',' + \
                                ...
                                deliveries_df['num_codigopostal'].astype(int).astype(str).str.strip() + ',Nuevo León, México'
